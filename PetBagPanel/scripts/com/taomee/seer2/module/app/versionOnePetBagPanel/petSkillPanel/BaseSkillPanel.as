package com.taomee.seer2.module.app.versionOnePetBagPanel.petSkillPanel {
   ...
   public class BaseSkillPanel extends Sprite {
      ...
      protected function onNormalSkillCellClick(evt:Event) : void {
         var targetCell:BaseSkillCell = evt.currentTarget as BaseSkillCell;
         if (this._selectedNormalSkillCell == targetCell) {
            this._selectedNormalSkillCell.isSelected = false;
            this._selectedNormalSkillCell = null;
         }
         else {
            this._selectedNormalSkillCell = targetCell;
            for each (var cell:BaseSkillCell in this._normalSkillCellVec) {
               cell.isSelected = (cell == targetCell);
            }
            this.dispatchEvent(new Event("requestChangeSkill"));
         }
      }
      
      protected function onCriticalSkillCellClick(evt:Event) : void {
         var target:BaseSkillCell = evt.currentTarget as BaseSkillCell
         if (this._selectedCriticalSkillCell == target) {
         ...
         this._selectedCriticalSkillCell = target;
      ...
      public function replaceCriticalSkill(info:SkillInfo) : void {
         ...
         if(this._selectedCriticalSkillCell != null) {
            this._selectedCriticalSkillCell.showReplaceAnimation();
         }
         this.resetSelectedCriticalSkillCell();
         this.updateDisplay();
      }
      ...
      public function replaceNormalSkill(info:SkillInfo) : void {
         ...
         if(this._selectedNormalSkillCell != null) {
            this._selectedNormalSkillCell.showReplaceAnimation();
         }
         this.resetSelectedNormalSkillCell();
         this.updateDisplay();
      }
