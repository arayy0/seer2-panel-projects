package fl.timeline
{
   ...
   public class TimelineManager
   {
      ...
      public function initInstance(instance:DisplayObject, parent:DisplayObjectContainer) : void
      {
         ...
         if(ii != null)
         {
            if(this.getInstanceForInfo(ii,instance) == null)
            {
               ...
               instance.addEventListener("frameConstructed",this.handleInit);
               instance.addEventListener("removed",this.handleInitRemoved);
            }
         }
      }
      ...
      public function getInstance(container:DisplayObject, instanceName:String, frame:int, sceneName:String = null) : DisplayObject
      {
         ...
         var containerIsBtn:* = container is SimpleButton;
         for(i = 0; i < list.length; )
         {
            ii = list[i];
      ...
      {
         e.target.removeEventListener("removed",this.handleInitRemoved);
         e.target.removeEventListener("frameConstructed",this.handleInit);
      }
      ...
      protected function getInstanceForInfoWithXns(ii:InstanceInfo) : DisplayObject
      {
         ...
         else
         {
            ...
            while(currentII != null && currentII != ii)
            {
               ...
               {
                  this.addXn(prevII.content,currentII.content,currentII,1);
                  this.addXn(currentII.content,prevII.content,prevII,0);
               }
               prevII = currentII;
            ...
            if(ii.content != null)
            {
               ...
               {
                  this.addXn(prevII.content,ii.content,ii,1);
                  this.addXn(ii.content,prevII.content,prevII,0);
               }
               if(ii.next != null)
               {
                  ...
                  if(ii.next.content != null)
                  {
                     this.addXn(ii.content,ii.next.content,ii.next,1);
                     this.addXn(ii.next.content,ii.content,ii,0);
                  }
               }
            }
         }
         if(this._supportParentChildAcrossFrames && ii.children != null)
         {
            ...
            for(i = 0; i < len; )
            {
               ...
               if(childII.content != null)
               {
                  this.addXn(ii.content,childII.content,childII,2);
                  this.addXn(childII.content,ii.content,ii,3);
               }
               i++;
            }
         }
         if(this._supportParentChildAcrossFrames && ii.parent != null)
         {
            ...
            {
               this.addXn(ii.content,ii.parent.content,ii.parent,3);
               this.addXn(ii.parent.content,ii.content,ii,2);
            }
         }
         if(!this._supportNextPrevAcrossFrames && (ii.next != null || ii.prev != null) || !this._supportParentChildAcrossFrames && (ii.children != null || ii.parent != null) || ii.xns != null && ii.xns.length > 0)
         {
            ...
            ii.content.addEventListener("frameConstructed",this.handleXns);
            if(ii.container is MovieClip)
            {
               ii.content.addEventListener("removed",this.handleRemoved,false,1);
            }
         }
         if(content == null)
         {
            ...
            for each(list in instanceDict)
            {
               for(i = 0; i < list.length; )
               {
                  ...
                  if(handledDict[ii] == undefined)
                  {
                     ...
                     {
                        xnList = ii.xns;
                        for(j = 0; j < xnList.length; )
                        {
                           xnII = xnList[j];
                           if(xnII.type >= 0 && xnII.type <= 3)
                           {
                              xnList.splice(j,1);
                              j--;
                           }
                           ...
                           {
                              for(k = 0; k < realIIForXnList.length; )
                              {
      ...
      {
         e.target.removeEventListener("removed",this.handleRemoved);
         e.target.removeEventListener("frameConstructed",this.handleXns);
      }
      ...
      protected function handleXns(e:Object, ii:InstanceInfo = null) : void
      {
         ...
         if(isSprite && isEvent)
         {
            content.removeEventListener("frameConstructed",this.handleXns);
         }
         i = 0;
         while(i < ii.xns.length)
         {
            xnII = ii.xns[i];
         ...
         if(!this._supportNextPrevAcrossFrames)
         {
            if(ii.prev != null)
            {
               this.handleXn(content,ii,ii.prev,0,currentFrame,isSprite);
            }
            if(ii.next != null)
            {
               this.handleXn(content,ii,ii.next,1,currentFrame,isSprite);
            }
         }
         if(!this._supportParentChildAcrossFrames)
         {
            if(ii.parent != null)
            {
               this.handleXn(content,ii,ii.parent,3,currentFrame,isSprite);
            }
            if(ii.children != null)
            {
               len = int(ii.children.length);
               for(j = 0; j < len; )
               {
                  ...
                  this.handleXn(content,ii,childII,2,currentFrame,isSprite);
                  j++;
               }
            }
         }
