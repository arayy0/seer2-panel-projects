package fl.text
{
   ...
   public class TLFRuntimeTabManager
   {
      ...
      private static function onAddedToStage(e:Event) : void
      {
         ...
         {
            displayObject.removeEventListener("addedToStage",onAddedToStage);
            if(!sTabHandlerInited)
            {
               displayObject.stage.addEventListener("keyFocusChange",onKeyFocusChange);
      ...
      public static function InitTabHandler(inDisplayObject:DisplayObject) : void
      {
         if(!sTabHandlerInited)
         {
            ...
            {
               inDisplayObject.stage.addEventListener("keyFocusChange",onKeyFocusChange);
               sTabHandlerInited = true;
            }
            else
            {
               inDisplayObject.addEventListener("addedToStage",onAddedToStage);
            }
         }
      }
   }
