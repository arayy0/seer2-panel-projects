   ...
   public class TCMText extends Sprite
   {
      ...
      private function invalidate() : void
      {
         ...
         if(stage != null)
         {
            try
            {
               stage.addEventListener("render",this.tlf_internal::repaint,false,0,true);
               stage.invalidate();
            }
            catch(se:SecurityError)
            {
               addEventListener("frameConstructed",tlf_internal::repaint,false,0,true);
            }
         }
         else if(parent == null)
         {
            addEventListener("added",this.tlf_internal::repaint,false,0,true);
         }
         else
         {
            addEventListener("addedToStage",this.tlf_internal::repaint,false,0,true);
            addEventListener("frameConstructed",this.tlf_internal::repaint,false,0,true);
         }
         this._invalid = true;
      }
      ...
      public function get maxScrollH() : int
      {
         ...
         var maxScroll:int = blockProg == "rl" ? int(bounds.height - this._tcm.compositionHeight) : int(bounds.width - this._tcm.compositionWidth);
      ...
      {
         var blockProg:String = this.UsingTextFlow() ? this._tcm.getTextFlow().blockProgression : this._tcm.hostFormat.blockProgression;
         return blockProg == "rl" ? int(Math.abs(this._tcm.verticalScrollPosition)) : int(Math.abs(this._tcm.horizontalScrollPosition));
      }
      ...
      tlf_internal function repaint(e:Event) : void
      {
         ...
         if(e != null)
         {
            if(e.type == "added" && e.target != this)
            {
               this._inRepaint = false;
               return;
            }
            removeEventListener("added",this.tlf_internal::repaint);
            removeEventListener("addedToStage",this.tlf_internal::repaint);
            removeEventListener("frameConstructed",this.tlf_internal::repaint);
            try
            {
               if(stage != null)
               {
                  stage.removeEventListener("render",this.tlf_internal::repaint);
               }
               else if(e.type == "render")
               {
                  e.target.removeEventListener("render",this.tlf_internal::repaint);
               }
            }
            catch(se:SecurityError)
            {
         ...
         if(this._background || this._border)
         {
            ...
            if(this._border)
            {
               graphics.lineStyle(this._borderWidth,this._borderColor,this._borderAlpha,false,"normal","none","miter",10);
            }
      ...
      {
         var blockProg:String = this.UsingTextFlow() ? this._tcm.getTextFlow().blockProgression : this._tcm.hostFormat.blockProgression;
         return blockProg == "rl" ? int(Math.abs(this._tcm.horizontalScrollPosition)) : int(Math.abs(this._tcm.verticalScrollPosition));
      }
      ...
      public function get maxScrollV() : int
      {
         var blockProg:String = this.UsingTextFlow() ? this._tcm.getTextFlow().blockProgression : this._tcm.hostFormat.blockProgression;
         var bounds:Rectangle = this._tcm.getContentBounds();
         var maxScroll:int = blockProg == "rl" ? int(bounds.width - this._tcm.compositionWidth) : int(bounds.height - this._tcm.compositionHeight);
         return maxScroll > 0 ? maxScroll : 0;
      }
      ...
      {
         return !(this._tcm.tlf_internal::composeState == 0 && this._tcm.tlf_internal::sourceState == 0);
      }
   }
}
