   ...
   public class TCMRuntimeManager extends TimelineManager
   {
      ...
      public static function checkTLFFontsLoaded(e:Event, fontName:String = null, fn:Function = null) : Boolean
      {
         ...
         if(fontName == null)
         {
            if(e != null)
            {
               e.target.removeEventListener("frameConstructed",checkTLFFontsLoaded,false);
            }
            return false;
         }
         loaded = true;
      ...
      {
         var firstFormat:ITextLayoutFormat = null;
         var xmlTree:XML = null;
         var editPolicy:String = null;
         var usingTextFlow:Boolean = false;
         var leaf:FlowLeafElement = null;
         var typeTextStr:String = null;
         var multipleFormats:Boolean = false;
         var hasAnchor:* = false;
         var hasTCY:* = false;
         var multipleParagraphs:* = false;
         var p:ParagraphElement = null;
         var hasILG:Boolean = false;
         var curLeaf:FlowLeafElement = null;
         ...
         textFlow = TextConverter.importToFlow(ii.data,"textLayoutFormat",IConfiguration(tlf_internal::getGlobalConfig()));
         ...
         multipleFormats = textFlow.direction == undefined ? firstFormat.direction == "rtl" : textFlow.direction != firstFormat.direction;
         ...
         if(!multipleFormats && !multipleParagraphs)
         {
            while(curLeaf)
            {
               ...
               multipleFormats = !TextLayoutFormat.isEqual(computedFormat,firstFormat) || computedFormat.backgroundColor != undefined && computedFormat.backgroundColor != "transparent";
         ...
         if(attr != null && attr.length > 0)
         {
            textFlow.renderingMode = attr == "advanced" ? "cff" : "normal";
         }
         attr = xmlTree.@embedFonts;
         if(attr != null && attr.length > 0)
         {
            textFlow.fontLookup = attr.toLowerCase() == "true" ? "embeddedCFF" : "device";
         }
         if(usingTextFlow)
         {
