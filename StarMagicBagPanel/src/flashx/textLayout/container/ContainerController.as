package flashx.textLayout.container
{
   ...
   public class ContainerController implements IInteractionEventHandler, ITextLayoutFormat, ISandboxSupport
   {
      ...
      public function scrollToRange(activePosition:int, anchorPosition:int) : void
      {
         ...
         var verticalText:* = this.tlf_internal::effectiveBlockProgression == BlockProgression.RL;
      ...
      tlf_internal function updateCompositionShapes() : void
      {
         ...
         if(this._updateStart > this.absoluteStart && newShapeChildren.length > 0)
         {
            ...
            shapeChildrenStartIdx = int(this._shapeChildren.indexOf(prevTextLine));
         ...
         while(newIdx != newShapeChildren.length)
         {
            ...
            {
               newChildIdx = int(this._shapeChildren.indexOf(newChild));
               if(newChildIdx == -1)
               {
         ...
         this._mouseEventManager.updateHitTests(this.tlf_internal::effectiveBlockProgression == BlockProgression.RL && this._hasScrollRect ? this._contentWidth : 0,scratchRectangle,tf,!!firstVisibleLine ? firstVisibleLine.absoluteStart : this._absoluteStart,!!lastVisibleLine ? lastVisibleLine.absoluteStart + lastVisibleLine.textLength - 1 : this._absoluteStart,needsCtrlKey);
         this._updateStart = this._rootElement.textLength;
         if(this._measureWidth || this._measureHeight)
         {
            this.tlf_internal::attachTransparentBackgroundForHit(false);
         }
         if(tf.tlf_internal::backgroundManager)
         {
      ...
      tlf_internal function updateGraphics(updateStart:int) : void
      {
         ...
         if(floatIndex > 0)
         {
            ...
            containerListIndex = int(this._floatsInContainer.indexOf(floatInfo.graphic));
         ...
         while(floatIndex < numFloats)
         {
            ...
            else
            {
               ...
               {
                  index = int(this._floatsInContainer.indexOf(float));
      ...
      tlf_internal function getUserStyleWorker(styleProp:String) : *
      {
         ...
         return !!this._rootElement ? this._rootElement.tlf_internal::getUserStyleWorker(styleProp) : undefined;
      }
