   ...
   public class BaseCompose
   {
      ...
      private function composeBlockElement(elem:FlowGroupElement, absStart:int) : Boolean
      {
         ...
         while(idx < elem.numChildren && (absStart <= this._stopComposePos || !this.parcelList.atLast()))
         {
            ...
            {
               boxLeftIndent = child.tlf_internal::getEffectivePaddingTop();
               boxRightIndent = child.tlf_internal::getEffectivePaddingBottom();
               boxTopIndent = child.tlf_internal::getEffectivePaddingRight();
               boxBottomIndent = child.tlf_internal::getEffectivePaddingLeft();
            }
            else
            {
               boxLeftIndent = child.tlf_internal::getEffectivePaddingLeft();
               boxRightIndent = child.tlf_internal::getEffectivePaddingRight();
               boxTopIndent = child.tlf_internal::getEffectivePaddingTop();
               boxBottomIndent = child.tlf_internal::getEffectivePaddingBottom();
            }
      ...
      public function composeTextFlow(textFlow:TextFlow, composeToPosition:int, controllerEndIndex:int) : int
      {
         ...
         this._releaseLineCreationData = textFlow.configuration.releaseLineCreationData && Configuration.tlf_internal::playerEnablesArgoFeatures;
      ...
      private function advanceToComposeStartPosition() : Boolean
      {
         ...
         if(curLine.controller.tlf_internal::numFloats)
         {
            if(this._measuring)
            {
               return false;
         ...
         for(var parcelIndex:int = -1; parcelIndex < startParcel; parcelIndex++)
         {
            ...
            numFloats = this._curParcel.controller.tlf_internal::numFloats;
            if(numFloats)
            {
      ...
      private function initializeContentBounds(lineIndex:int, lastLineToCheck:int) : void
      {
         ...
         if(this._measuring)
         {
            while(lineIndex <= lastLineToCheck)
            {
               ...
               lineExtent = line.tlf_internal::lineExtent;
      ...
      tlf_internal function computeTextFlowLineMinimumLogicalTop(line:TextFlowLine, textLine:TextLine) : Number
      {
         ...
         if(line.tlf_internal::hasGraphicElement)
         {
            ...
            adjustedAscent = line.tlf_internal::getLineTypographicAscent(leafElement,leafElement.getAbsoluteStart(),textLine);
      ...
      protected function preProcessILGs(startPos:int) : Boolean
      {
         ...
         this._forceILGs = this._parcelList.explicitLineBreaks || verticalText && this._curParcel.controller.tlf_internal::measureHeight || !verticalText && this._curParcel.controller.tlf_internal::measureWidth;
      ...
      protected function composeInlineGraphicElement(inlineGraphic:InlineGraphicElement, textLine:TextLine) : Boolean
      {
         ...
         this._curParcel.controller.tlf_internal::addFloatAt(this._curParaStart + inlineGraphic.getElementRelativeStart(this._curParaElement),inlineGraphic.graphic,Float.NONE,marginAndPaddingX,marginAndPaddingY,!!fteInline ? fteInline.alpha : 1,!!fteInline ? fteInline.transform.matrix : null,this._parcelList.totalDepth,0,this._curParcel.columnIndex,textLine);
         return true;
      }
      ...
      private function getRightSideGap(curLine:TextFlowLine, aligned:Boolean) : Number
      {
         ...
         if(paraFormat.direction == Direction.RTL && Boolean(curLine.location & TextFlowLineLocation.FIRST))
         {
            ...
            if(curLine.tlf_internal::hasNumberLine && elem.getParentByType(ListItemElement).computedFormat.listStylePosition == ListStylePosition.INSIDE)
            {
      ...
      protected function finishComposeLine(curTextLine:TextLine, numberLine:TextLine) : void
      {
         ...
         else
         {
            ...
            {
               curLineAscent = this._curLine.tlf_internal::getLineTypographicAscent(this._curElement,this._curElementStart,curTextLine);
               if(numberLine)
               {
            ...
            else if(baselineType == BaselineOffset.LINE_HEIGHT)
            {
               ...
               {
                  curLeadingDirectionUp = this._atColumnStart ? true : ParagraphElement.tlf_internal::useUpLeadingDirection(this._curLineLeadingModel);
                  prevLeadingDirectionUp = this._atColumnStart || this._lastLineLeadingModel == "" ? true : ParagraphElement.tlf_internal::useUpLeadingDirection(this._lastLineLeadingModel);
      ...
      protected function finishController(controller:ContainerController) : void
      {
         ...
         {
            paddingLeft = controller.tlf_internal::getTotalPaddingLeft();
            paddingTop = controller.tlf_internal::getTotalPaddingTop();
            paddingRight = controller.tlf_internal::getTotalPaddingRight();
            paddingBottom = controller.tlf_internal::getTotalPaddingBottom();
            if(this._blockProgression == BlockProgression.TB)
            {
      ...
      protected function parcelHasChanged(newParcel:Parcel) : void
      {
         ...
         if(newController)
         {
            this._verticalSpaceCarried = this._blockProgression == BlockProgression.RL ? newController.tlf_internal::getTotalPaddingRight() : newController.tlf_internal::getTotalPaddingTop();
            this._measuring = this._blockProgression == BlockProgression.TB && newController.tlf_internal::measureWidth || this._blockProgression == BlockProgression.RL && newController.tlf_internal::measureHeight;
         }
      }
      ...
      protected function pushInsideListItemMargins(numberLine:TextLine) : void
      {
         ...
         {
            numberLineWidth = TextFlowLine.tlf_internal::getNumberLineInsideLineWidth(numberLine);
