   ...
   internal class TextLayoutExporter extends BaseTextLayoutExporter
   {
         ...
         for(key in styles)
         {
            ...
            if(!(Boolean(exclusions) && exclusions.indexOf(val) != -1))
            {
               ...
               if(!prop)
               {
                  ...
                  {
                     if(val is String || Boolean(val.hasOwnProperty("toString")))
                     {
                        sortableStyles.push({
                           "xmlName":key,
                           "xmlVal":val
      ...
      override protected function exportFlowElement(flowElement:FlowElement) : XMLList
      {
         ...
         if(allStyles)
         {
            ...
            sortableStyles = this.tlf_internal::createStylesFromDescription(allStyles,this.formatDescription,true,!!flowElement.parent ? null : [FormatValue.INHERIT]);
